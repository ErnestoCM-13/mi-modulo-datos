...
from app.db import init_sqlite, init_duckdb, get_sqlite, get_duckdb, close_all
...
async def lifespan(app):
    ...
    init_sqlite(os.environ.get("SQLITE_PATH", "data/transactions.db"))
    init_duckdb(os.environ.get("PARQUET_PATH", "../data/transactions_1m_none.parquet"))

    # Pre-warm DuckDB: forces the reading of the Parquet to memory
    get_duckdb().execute("SELECT COUNT(*) FROM transactions").fetchone()
...
def batch_insert(transactions: list[TransactionIn]):
    ...
    # Deduplicar dentro del batch (quedarse con la primera ocurrencia)
    seen   = set()
    unique = []
